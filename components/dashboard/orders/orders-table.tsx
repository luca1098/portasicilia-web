...
import type { AdminOrder, GetAdminOrdersParams, OrderSort, OrderStatus } from '@/lib/api/orders'
import OrderDetailDialog from './order-detail-dialog'

type OrdersTableProps = {
  initialOrders: AdminOrder[]
  initialNextCursor: string | null
  fetchParams: GetAdminOrdersParams
}
...
export default function OrdersTable({ initialOrders, initialNextCursor, fetchParams }: OrdersTableProps) {
  ...
  function handleUpdateStatus(orderId: string, status: OrderStatus) {
    executeStatusUpdate(() => updateOrderStatusAction(orderId, status))
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start gap-2">
        <form onSubmit={handleSearchSubmit} className="flex flex-1 items-center gap-2">
          <InputWrapper
            label={t.admin_orders_search_placeholder}
          ...
          <Button type="submit" disabled={filtering}>
            ...
            {t.admin_orders_search_button}
          </Button>
        </form>
        <InputWrapper label={t.admin_orders_sort_label} hasValue className="w-[180px]">
          <Select value={sort} onValueChange={handleSortChange} disabled={filtering}>
            <SelectTrigger className="w-full">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="newest">{t.admin_orders_sort_newest}</SelectItem>
              <SelectItem value="oldest">{t.admin_orders_sort_oldest}</SelectItem>
            </SelectContent>
          </Select>
        </InputWrapper>
      </div>
      ...
      {orders.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border/60 bg-muted/20 px-6 py-14 text-center">
          ...
          <p className="max-w-[260px] text-sm leading-relaxed text-muted-foreground/70">
            {appliedSearch ? t.admin_orders_search_no_results : t.admin_orders_no_results}
          </p>
        </div>
      ) : (
        <>
