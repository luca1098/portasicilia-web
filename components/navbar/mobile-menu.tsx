        ...
        <div className="flex h-full flex-col overflow-y-auto">
          ...
          <nav className="flex flex-col px-5 py-6">
            {items.map((item, index) => (
              ...
              >
                {t[item.key] ?? item.key}
                {isActiveLink(item.href) && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-primary" />}
              </Link>
            ))}
            ...
            <div
              ...
              className="animate-in fade-in slide-in-from-right-4 fill-mode-both border-b border-border/50"
            >
              <button
              ...
              >
                {t.get_inspired}
                <ChevronDownIcon
                  className={cn('h-4 w-4 transition-transform duration-300', categoriesOpen && 'rotate-180')}
                />
              </button>
