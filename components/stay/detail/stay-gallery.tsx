...
export default function StayGallery({ images, alt, video }: StayGalleryProps) {
        ...
        <div className="hidden gap-2 md:flex" style={{ height: GRID_HEIGHT }}>
          ...
          <div className="grid flex-1 grid-cols-2 grid-rows-2 gap-2">
            {sideImages.map((image, idx) => {
              ...
              const isLast = idx === 3
              return (
                <div key={`img-${idx}`} className={`relative overflow-hidden ${cornerClass}`}>
                  <Image
                    src={image}
                    alt={`${alt} ${idx + 1}`}
                    fill
                  ...
                  {isLast && remainingCount > 0 && (
                    ...
                    >
                      {interpolate(t.stay_detail_photos_more, { count: String(remainingCount) })}
                    </button>
                  )}
                </div>
              )
        ...
        <Dialog open={videoOpen} onOpenChange={setVideoOpen}>
          ...
          >
            <div className="flex items-center justify-between bg-black px-4 py-3 md:px-8">
              ...
              <DialogClose
                aria-label={t.stay_detail_close}
