...
function ReviewCard({ review }: { review: Review }) {
  ...
  const timeAgo = getRelativeTime(review.createdAt, {
    justNow: t.stay_detail_time_just_now,
    daysAgo: t.stay_detail_time_days_ago,
    weeksAgo: t.stay_detail_time_weeks_ago,
    monthsAgo: t.stay_detail_time_months_ago,
  })
  ...
  return (
    <div className="rounded-xl border p-4">
      <div className="mb-3 flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-semibold">{review.userName ?? t.stay_detail_anonymous}</p>
...
export default function StayReviews({ reviews }: StayReviewsProps) {
  const t = useTranslation()
    ...
    <div>
      <div className="mb-6 flex items-center gap-2">
        ...
        <h2 className="text-xl font-bold">
          {interpolate(t.stay_detail_reviews_count, {
      ...
      {reviews.length > VISIBLE_LIMIT && (
        <Button variant="outline" className="mt-6 w-full" onClick={() => null}>
          {t.stay_detail_show_all_reviews}
        </Button>
      )}
    </div>
  )
